...
RUN echo 'lw117' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo 'whoami ' >>/lw117.sh

RUN echo 'cd ' >>/lw117.sh

RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/lw117.sh

RUN echo 'cd /noVNC-1.2.0' >>/lw117.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/lw117.sh

RUN chmod 755 /lw117.sh
